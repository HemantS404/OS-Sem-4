...
struct process{
    int arrival, burst, completion, turnaround, waiting, completed, r_burst, start;
};
...
int getMinProcess(int clock, int total){
    ...
    for(int i=minProcess; i<total; i++){
        if (pq[i].arrival <= clock && pq[i].completed != 1 && pq[i].r_burst < pq[minProcess].r_burst){
            minProcess = i;
        }
    }
    return minProcess;
...
void main(){
    ...
    printf("This is SRTF process sheduling\n");
